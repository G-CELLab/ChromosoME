...
/// All scene context comes from TutorSceneState. All knowledge comes from RAG.
/// </summary>
public class AIResponseGenerator : MonoBehaviour
{
    ...
    private const string SYSTEM_TEMPLATE =
        ...
        "Use a warm, encouraging tone. Answer biology questions clearly and accurately. " +
        "Use the knowledge base excerpts for detailed information and scene guidance. " +
        "If asked what to do, guide the learner using the knowledge base for the current phase. " +
        ...
        Action<string>     onComplete)
    {
        ...
        {
            Debug.LogError("[AIResponseGenerator] No API key found.");
            onComplete?.Invoke("[Error: API key not configured]");
            yield break;
        }
    ...
    /// <summary>
    /// User message = scene context from TutorSceneState + the learner's query.
    /// </summary>
    private static string BuildUserMessage(string query, TutorSceneState state)
    {
        return $"{state.ToContextString()}\n\nStudent: {query}";
    }
